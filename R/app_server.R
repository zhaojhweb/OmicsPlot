...
app_server <- function(input, output, session) {
  ...
  observeEvent(input$Calblup,{
    withProgress(message = "Calculating",
                 ...
                 expr = {
                   ...
                   blup_data <- reactive({
                     data <- blup(phenodata(), sample = "Line", loc = "Loc", rep = "Rep", year = "Year", phe = "Phe", fold = 1.5)
                   })
  ...
  observeEvent(input$Cal_corr, {
    ...
    output$download_Cor_heatmap <- downloadHandler(
      filename = function(){
        paste0("Corr_",input$Cor_method,".", input$Cor_heatmap_format, sep = "")
      },
      content = function(file){
        withProgress(message = "Downloading",
                     detail = "It will take some time ^-^",
  ...
  observeEvent(input$Hcluster_plot,{
    ...
    output$download_Hcluster <- downloadHandler(
      filename = function(){
        paste0("Hcluster.",input$Hcluster_format, sep ="")
      },
      content = function(file){
        withProgress(
          message = "Downloading",
          ...
          expr = {
            ...
            if(input$Hcluster_format == "jpeg"){
              jpeg(file, height = input$Hcluster_height, width = input$Hcluster_width, units = "in", res = 800)
            }else if(input$Hcluster_format == "pdf"){
              pdf(file, height = input$Hcluster_height, width = input$Hcluster_width)
            }else if(input$Hcluster_format == "png"){
              png(file, height = input$Hcluster_height, width = input$Hcluster_width, units = "in", res = 800)
            }else if(input$Hcluster_format == "svg"){
              svg(file, height = input$Hcluster_height, width = input$Hcluster_width)
            }else if(input$Hcluster_format == "tiff"){
              tiff(file, height = input$Hcluster_height, width = input$Hcluster_width)
            }
            trait_hcluster <- hclust(dist(t(traitdata())))
            plot(trait_hcluster)
            dev.off()
  ...
  de_result <- reactive({
    ...
    de_result <- read.delim(input$UploadVolcano$datapath, header = T,
                            sep = "\t")
    de_result_dir <- mutate(de_result,direction = if_else(padj > 0.05, 'no_change', if_else(
      abs(log2FoldChange) < 1, 'ns', if_else(
        log2FoldChange >= 1, 'up', 'down'
      )
    )))
    return(de_result_dir)
  })
  ...
  observeEvent(input$Volcano_plot,{
    ...
    output$download_Volcano <- downloadHandler(
      filename = function(){
        paste0("Volcano.",input$Volcano_format, sep ="")
      },
      content = function(file){
        withProgress(
          message = "Downloading",
          ...
          expr = {
            ...
            if(input$Volcano_format == "jpeg"){
              jpeg(file, height = input$Volcano_height, width = input$Volcano_width, units = "in", res = 800)
            }else if(input$Volcano_format == "pdf"){
              pdf(file, height = input$Volcano_height, width = input$Volcano_width)
            }else if(input$Volcano_format == "png"){
              png(file, height = input$Volcano_height, width = input$Volcano_width, units = "in", res = 800)
            }else if(input$Volcano_format == "svg"){
              svg(file, height = input$Volcano_height, width = input$Volcano_width)
            }else if(inputVolcano_format == "tiff"){
              tiff(file, height = input$Volcano_height, width = input$Volcano_width)
            }
  ...
  gene_expr <- reactive({
    ...
    expr1 <- read.delim(input$Uploadheatmap$datapath, header = T, row.names = 1)
    return(expr1)
  })
  observeEvent(input$heatmap_plot,{
    ...
    output$download_heatmap <- downloadHandler(
      filename = function(){
        paste0("Heatmap.", input$heatmap_format, sep = "")
      },
      content = function(file){
        withProgress(
          message = "Downloading",
          ...
          expr = {
            ...
            if(input$heatmap_format == "jpeg"){
              jpeg(file, height = input$heatmap_height, width = input$heatmap_width, units = "in", res = 800)
            }else if(input$heatmap_format == "pdf"){
              pdf(file, height = input$heatmap_height, width = input$heatmap_width)
            }else if(input$heatmap_height == "png"){
              png(file, height = input$heatmap_height, width = input$heatmap_width, units = "in", res = 800)
            }else if(input$heatmap_format == "svg"){
              svg(file, height = input$heatmap_height, width = input$heatmap_width)
            }else if(input$heatmap_format == "tiff"){
              tiff(file, height = input$heatmap_height, width = input$heatmap_width)
            }
            pheatmap(
              gene_expr(),
              scale = input$heatmap_scale,
